...
const INDICES_TO_FILTER = ATHLETE_DATA_FILTERED_FIELDNAMES.map((field) =>
    ATHLETE_DATA_FIELDNAMES.indexOf(field)
);
...
export default function Database() {
    ...
    const filterHeaderStats = (headerStats: string[]) => {
        return headerStats.filter((header) =>
            ATHLETE_DATA_FILTERED_FIELDNAMES.includes(header)
        );
    };

    const filterRowData = (rowData: string[][]) => {
        return rowData.map((row) =>
            INDICES_TO_FILTER.map((index) => row[index])
        );
    };
